/*
 ...
 Thanks to Greg Shakar and Scott Fitzgerald for the improvements
   ...
   The circuit:
 * potentiometers attached to pins 2 and 3 
 * pushbutton attached to pin 5
...
void setup()
{
  ...
  pinMode(5, INPUT);   // digital sensor is on digital pin 2
...
void loop()
{
  ...
  if (Serial.available() > 0) {
    ...
    firstSensor = analogRead(2)/4;
    // delay 10ms to let the ADC recover:
    delay(10);
    // read second analog input, divide by 4 to make the range 0-255:
    secondSensor = analogRead(3)/4;
    // read  switch, map it to 0 or 255L
    thirdSensor = map(digitalRead(5), 0, 1, 0, 255);  
    // send sensor values:
